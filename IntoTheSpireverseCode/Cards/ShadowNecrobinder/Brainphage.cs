...
public sealed class Brainphage() : ShadowNecrobinderCard(0, CardType.Skill, CardRarity.Rare, TargetType.Self)
{
    ...
    public override async Task AfterCardChangedPiles(CardModel card, PileType oldPile, AbstractModel? source)
    {
        ...
        {
            await CardPileCmd.AddCursesToDeck(Enumerable.Repeat(ModelDb.Card<SporeMind>(), 1), Owner);
        }
    }
}
